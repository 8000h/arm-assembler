...
static bool grammar_rest(void);
static bool grammar_colon(void);
static bool grammar_after(void);
...
static bool grammar_rest(void)
{
	...
	if (!grammar_colon())
	{
		strncpy(current_line_info->instr, str, 16);
		status = grammar_ops();
	}
...
static bool grammar_colon(void)
{
	...
	if (accept(TOK_COLON))
	{
		strncpy(current_line_info->label, str, 16);
		current_line_info->label_defined = true;
		status = grammar_after();
	}
